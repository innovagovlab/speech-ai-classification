...
try:
    ...
    cursor.execute("SELECT id, transcription, post_description, tone_deepseek_zs FROM data")
    ...
    for row in rows:
        if row[3] == "":
            ...
            prompt = set_prompt_with_description("tone_classification/prompts/tone-zero-shot.txt", row[1], row[2])
            ...
            cursor.execute(
                "UPDATE data SET tone_deepseek_zs = %s WHERE id = %s",
